...
def process_single_file(fpath):
    ...
    # parse filename (no more _slide suffix: one file now bundles every shortslide
    # produced internally by --do-shortslides for that (bank, window))
    basename = os.path.basename(fpath)
    match = re.search(r'_bg_bank\d+_(\d+)-(\d+)\.hdf', basename)
    if match:
        segment = (int(match.group(1)), int(match.group(2)))
...
def main():
    ...
    parser.add_argument("--segment-length", default=256, type=int,
                         help="Must match the --segment-length used in GWsig_prep.py / the .sh job.")
    parser.add_argument("--slide-shift", default=0.503, type=float,
    ...
    T_bg = 0
    for (start_t, end_t) in analyzed_segments:
