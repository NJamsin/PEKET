...
def main():
    ...
    parser.add_argument("--prep-script", default="gw-sig-prep2", help="Name of the preparation script")
    parser.add_argument("--post-script", default="gw-sig-post2", help="Name of the post-processing script")
    ...
    parser.add_argument("--target-sigma", default=5, choices=[1,2,3,4,5], type=int, help="Overwrite \"slide-shift\" and \"segment-length\" to ensure accumulatin enough background to reach a X sigma FAP.")
    ...
    def write_sub_file(sub_name, cmd_name):
        ...
        if sub_name == "sig_prep.sub":
            cmd_args += f" --window {args.window} --max-extension {args.max_extension} --OSW-sigma {args.OSW_sigma} --chunk-size {args.chunk_size} --segment-length {args.segment_length} --n-ifos {args.n_ifos} --target-sigma {args.target_sigma}"
            if args.ldg_tag: cmd_args += f" --ldg-tag {args.ldg_tag}"
            mem = "2GB"
            disk = "2GB"
        elif sub_name == "sig_post.sub":
            cmd_args += f" --OSW-sigma {args.OSW_sigma} --fit-steps {args.fit_steps} --exclude-top-steps {args.exclude_top_steps} --segment-length {args.segment_length} --slide-shift {args.slide_shift} --n-ifos {args.n_ifos}"
            mem = "1GB"
            disk = "500MB"
...
# --- MATH BLOCK FOR EXACT CHUNKING ---
    try:
        ...
        total_jobs = 0
        if args.window in ['both', 'before']: total_jobs += w1 * num_banks
        if args.window in ['both', 'after']: total_jobs += w2 * num_banks
        
        num_chunks = max(1, math.ceil(total_jobs / args.chunk_size))
        print(f"Estimated {total_jobs} background search jobs (was previously multiplied "
              f"by n_slides; shortslides now happen inside each job instead).")
        ...
        try:
            ...
            with open(window_file, 'r') as wf:
                for line in wf:
                    ...
                    if len(parts) >= 3:
                        bank_num, start_time, end_time = parts[0], parts[1], parts[2]
                        # Construction du nom de fichier exact généré par le .sh
                        expected_file = f"{suffix}_bg_bank{bank_num}_{start_time}-{end_time}.hdf"
