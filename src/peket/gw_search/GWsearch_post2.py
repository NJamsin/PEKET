...
def main():
    ...
    if args.plot_spectrogram:
        ...
        # Auto-detect which detectors actually took part in the winning trigger's
        # job. Since jobs are now generated per coincident segment (2 or 3 out of
        # H1/L1/V1 depending on which were on), we can't assume H1+L1 anymore --
        # each trigger hdf5 file only contains groups for the IFOs used in that job.
        with h5py.File(best_cand['file'], 'r') as bf:
            candidate_ifos = [k for k in bf.keys() if k != 'network']
        print(f" -> Detectors present in this trigger's job: {candidate_ifos}")

        from gwpy.timeseries import TimeSeries

        for label in candidate_ifos:
            channel = f"{label}:GWOSC-4KHZ_R1_STRAIN"
            cache_file = f"{BASE_DIR}/data/{SUFFIX}_{label}.lcf"
